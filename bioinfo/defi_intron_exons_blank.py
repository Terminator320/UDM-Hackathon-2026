...
class SplicingDecoder:
    ...
    def process_base(self, base):
        ...
        if self._in_intron:
            self._intron_buffer.append(b)
            if (
                len(self._intron_buffer) >= 2
                and self._intron_buffer[-2] == "A"
                and self._intron_buffer[-1] == "G"
            ):
                intron = "".join(self._intron_buffer)
                self.introns_found.append(intron)
                events.append(("INTRON_END", intron))
                self._in_intron = False
                self._intron_buffer = []
            return events
        ...
        if self._pending_g:
            if b == "U":
                self._in_intron = True
                self._pending_g = False
                self._intron_buffer = ["G", "U"]
                events.append(("INTRON_START",))
                return events

            self._handle_exon_base("G", events)
            self._pending_g = False

            if b == "G":
                self._pending_g = True
            else:
                self._handle_exon_base(b, events)
            return events
    ...
    def  _handle_exon_base(self, base, events):
