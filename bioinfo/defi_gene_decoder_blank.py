...
class GeneStatistics:
    ...
    def get_avg_gene_length(self):
        if self.total_genes == 0:
            return 0.0
        return self.total_gene_codons / self.total_genes

    def get_quality_score(self):
        if self.total_bases == 0:
            return 0.0
        invalid_ratio = self.invalid_bases / self.total_bases
...
class StreamingGenomeDecoder:
    ...
    def is_valid_base(self, base):
        # do it
        return base.upper() in ("A","U","G","C")

    def process_base(self, base):
        #do it ;)
        events = []

        # total bases (raw stream)
        ...
        self.stats.gc_content = (self._gc_count / self._valid_bases) * 100.0

        # build codons
        self._codon_buffer.append(base.upper())
        ...
        if token == "START":
            if not self._in_gene:
                self._in_gene = True
                self._current_gene = [codon]
                events.append(("START",))
            else:
                self._current_gene.append(codon)

            events.append(("", list(self._current_gene)))

        elif token == "STOP":
            if self._in_gene:
                self._current_gene.append(codon)
                finished_gene = list(self._current_gene)
                self.all_genes.append(finished_gene)
                self.stats.total_genes += 1

                gene_len = max(len(finished_gene) - 2, 0)
                self.stats.total_gene_codons += gene_len

                events.append(("STOP", finished_gene))
                self._in_gene = False
                self._current_gene = []
        else:
            if self._in_gene:
    ...
    with Live(console=console, refresh_per_second=10) as live:
        for base in genome:
            ...
            layout.split_column(
                ...
                Layout(render_gene_panel(decoder._current_gene), size=5),
